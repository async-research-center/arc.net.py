...
class LocalDatabase:
    ...
    def load_default_docs(self):
        self.docs["SCP-001"]=SCPDocument("SCP-001","Special Containment Procedures – SCP-001",textwrap.dedent("""SCP‑001 FILE..."""))
        self.docs["SCP-002"]=SCPDocument("SCP-002","Special Containment Procedures – SCP-002",textwrap.dedent("""SCP‑002 FILE..."""))
        self.docs["SCP-003"]=SCPDocument("SCP-003","Special Containment Procedures – SCP-003",textwrap.dedent("""SCP‑003 FILE..."""))
    def list_docs(self):
        print("\n=== SCP INDEX ===")
    ...
    def view_doc(self,i):
        d=self.docs.get(i)
        if not d: print("NO MATCH.\n"); return
        print(f"\n=== {d.title} ===\n{d.content}\n")

# ===== V-MAIL =====
FAKE_USER_EMAIL="dr.crowley@foundation.int"
...
class VMailBox:
    ...
    def _add(self,sender,recipient,subject,body,folder):
        m=VMailMessage(self.next_id,sender,recipient,subject,body,datetime.now().strftime("%Y-%m-%d %H:%M:%S"),folder)
        self.messages.append(m); self.next_id+=1; return m
    def send_to_owner(self,sub,body): return self._add(f"{DISPLAY_NAME}@o5.foundation.int",self.owner,sub,body,"INBOX")
    ...
    def list(self,f):
        ...
        for m in self.messages:
            if m.folder==f:
                found=True; print(f"{m.msg_id:03d} :: {m.timestamp} :: {m.sender} -> {m.recipient} :: {m.subject}")
        print("NO MESSAGES.\n" if not found else "")
    def read(self,i):
        for m in self.messages:
            if m.msg_id==i:
    ...
    def archive(self,i):
        for m in self.messages:
            if m.msg_id==i: m.folder="ARCHIVE"; print("ARCHIVED.\n"); return
        print("NO SUCH MESSAGE.\n")
    def delete(self,i):
        for x,m in enumerate(self.messages):
            if m.msg_id==i: del self.messages[x]; print("DELETED.\n"); return
        print("NO SUCH MESSAGE.\n")
...
class O5:
    def __init__(s):
        ...
        s.session_start=datetime.now(); s.vmail=VMailBox(FAKE_USER_EMAIL)
        s.alerts=[]; s.simulations=[]; s.redactions=[]; s.checklists=[]
        s.metrics={"commands":0}; s.last=[]
    def _cmd(s,n): s.metrics["commands"]+=1; s.last.append(n); s.last=s.last[-10:]

    # Modules 1–11 unchanged but minimized
    def f1(s): s._cmd("1"); print(f"\n[01] OFFLINE STATUS: {s.offline}\n")
    def f2(s): s._cmd("2"); print("\n[02] ONLINE: DISABLED (OFFLINE)\n")
...
# ===== MAIN (MENU SHOWS ONLY ONCE + ON HELP) =====
def main():
